...
router.get('/accounts', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const enriched = list.map((acc) => {
      ...
      let debits = b?.debits || 0;
      let credits = b?.credits || 0;

      // Incorporate openingBalance from accounts table
      const isDebitBook = debitNormalTypes.has(acc.type);
      const isContraAsset = acc.type === 'asset' && (acc.name.toLowerCase().includes('accumulated depreciation') || acc.name.toLowerCase().includes('accumulated amortisation'));
      const isDebitBookOpening = (acc.type === 'asset' && !isContraAsset) || acc.type === 'expense';
      if (acc.openingBalance !== 0) {
        const ob = acc.openingBalance;
        if (isDebitBookOpening) {
          if (ob > 0) debits += ob;
          else credits += Math.abs(ob);
        } else {
          if (ob > 0) credits += ob;
          else debits += Math.abs(ob);
        }
      }

      // Fixed assets override
      const faData = faMap.get(acc.id);
      if (faData && acc.type === 'asset') {
      ...
      // Inventory override
      if (inventoryValue > 0 && invAccount && acc.id === invAccount.id) {
        const jeBalance = debits - credits;
      ...
      const balance = isDebitBook ? debits - credits : credits - debits;
      return { ...acc, balance };
    });
...
router.post('/accounts', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    await createAuditLog({ orgId, userId: req.user!.userId, action: 'create', entityType: 'account', entityId: account.id, newValues: { code: body.code, name: body.name }, ...extractReqMeta(req) });
...
router.patch('/accounts/:id', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!account) throw new AppError('Account not found.', 404);
    await createAuditLog({ orgId, userId: req.user!.userId, action: 'update', entityType: 'account', entityId: id, newValues: body, ...extractReqMeta(req) });
...
router.post('/accounts/seed', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    await createAuditLog({ orgId, userId: req.user!.userId, action: 'seed', entityType: 'account', newValues: { seeded: true }, ...extractReqMeta(req) });
...
router.delete('/accounts/:id', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    await createAuditLog({ orgId, userId: req.user!.userId, action: 'delete', entityType: 'account', entityId: id, ...extractReqMeta(req) });
...
router.post('/accounts/import-csv', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      const [account] = await db
        .insert(accounts)
        .values({ orgId, code, name, type, subType, parentId, description, isActive, isSystem: false, openingBalance } as any)
        .returning();
    ...
    await createAuditLog({ orgId, userId: req.user!.userId, action: 'import', entityType: 'account', newValues: { count: created.length }, ...extractReqMeta(req) });
...
router.post('/post-opening-balances', requireRole('owner', 'admin'), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    await createAuditLog({ orgId, userId: req.user!.userId, action: 'post', entityType: 'opening-balance', newValues: { posted: true }, ...extractReqMeta(req) });
