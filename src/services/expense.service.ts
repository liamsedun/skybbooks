...
import { eq, and, sql, gte, lte, gt, getTableColumns } from 'drizzle-orm';
import {
  db,
  accounts,
  expenses,
...
export async function listExpenses(orgId: string, filters?: { startDate?: Date; endDate?: Date }): Promise<any[]> {
  ...
  return await db
    .select({
      ...
      creditAccountName: accounts.name,
    })
    .from(expenses)
  ...
  return await db.transaction(async (tx) => {
    ...
    const [updatedExpense] = await tx
      ...
      .set({
        ...
        projectId: input.projectId !== undefined ? input.projectId : expense.projectId,
