  ...
  return generatePDFBuffer((doc) => {
    ...
    if (payments.length > 0) {
      ...
      for (const pmt of payments) {
        ...
        doc.text(formatNaira(pmt.amountAllocated as number), summaryValX, y, { align: 'right' });
        y += 10;
      }
...
export async function generatePayslipPDF(payrollLineId: string): Promise<Buffer> {
  ...
  return generatePDFBuffer((doc) => {
    ...
    // ── Modern Header ──
    doc.rect(startX, y, pageW, 40).fill(DARK_HEADER);
    // Logo
    if (logoBuffer) {
      doc.save();
      doc.circle(startX + 24, y + 20, 16);
      doc.clip();
      doc.image(logoBuffer, startX + 10, y + 6, { width: 28, height: 28 });
      doc.restore();
    ...
    const empBottom = employee.address ? y + 44 : employee.phone ? y + 36 : employee.email ? y + 28 : y + 20;
    y = empBottom + 6;
    ...
    function drawCard(d: any, cx: number, cy: number, cw: number, rows: { l: string; v: string; b?: boolean; ital?: boolean }[], total?: { l: string; v: string }) {
      d.roundedRect(cx, cy, cw, 3, 3).fillColor('#fff').fill();
      d.roundedRect(cx, cy, cw, 3, 3).lineWidth(0.5).strokeColor(LIGHT_BORDER).stroke();
      d.fontSize(7).fillColor(TEXT_PRIMARY);
      rows.forEach((r, idx) => {
        const ry = cy + 4 + idx * 10;
        ...
        d.text(r.l, cx + 7, ry + 1.5);
        d.font(r.b ? 'Helvetica-Bold' : 'Helvetica');
        d.text(r.v, cx + cw - 7, ry + 1.5, { align: 'right' });
      });
      d.fillColor('#ffffff').fill();
      if (total) {
        const ty = cy + 4 + rows.length * 10;
        d.rect(cx, ty, cw, 11).fillColor('#f1f5f9').fill();
        d.lineWidth(1).strokeColor('#1e3a5f').moveTo(cx + 4, ty - 0.5).lineTo(cx + cw - 4, ty - 0.5).stroke();
        d.fillColor('#1e3a5f').font('Helvetica-Bold').fontSize(7);
        d.text(total.l, cx + 7, ty + 2);
        d.text(total.v, cx + cw - 7, ty + 2, { align: 'right' });
        return ty + 11;
      }
      return cy + 4 + rows.length * 10;
    }
    ...
    if (bandRows.length > 0) {
      ...
      doc.roundedRect(tx, y, tw, 12, 3).fillColor('#1e3a5f').fill();
      doc.fillColor('#ffffff').font('Helvetica-Bold').fontSize(5.5);
      doc.text('Band', tx + 6, y + 3.5);
      doc.text('Rate', tx + bw[0] + 6, y + 3.5, { align: 'right', width: bw[1] - 12 });
      doc.text('Taxable Amount', tx + bw[0] + bw[1] + 6, y + 3.5, { align: 'right', width: bw[2] - 12 });
      doc.text('Tax', tx + bw[0] + bw[1] + bw[2] + 6, y + 3.5, { align: 'right', width: bw[3] - 12 });
      y += 12;

      bandRows.forEach((b, idx) => {
        doc.rect(tx, y, tw, 9).fillColor(idx % 2 === 0 ? '#ffffff' : '#f8fafc').fill();
        doc.rect(tx, y, tw, 9).lineWidth(0.5).strokeColor('#eef2f6').stroke();
        doc.fillColor(TEXT_PRIMARY).font('Helvetica').fontSize(5.5);
        doc.text(b.name, tx + 6, y + 1.5);
        doc.text(`${(b.rate * 100).toFixed(0)}%`, tx + bw[0] + 6, y + 1.5, { align: 'right', width: bw[1] - 12 });
        doc.text(formatNaira(b.taxable), tx + bw[0] + bw[1] + 6, y + 1.5, { align: 'right', width: bw[2] - 12 });
...
export async function generateBillPDF(billId: string, orgId: string): Promise<Buffer> {
  ...
  return generatePDFBuffer((doc) => {
    ...
    if ((bill as any).notes) { y += 20; doc.fillColor(MUTED).fontSize(8).font('Helvetica').text('Notes:', startX, y); doc.fillColor(TXT).text((bill as any).notes, startX, y+10, { width: 515 }); }
...
export async function generateCreditNotePDF(cnId: string, orgId: string): Promise<Buffer> {
  ...
  return generatePDFBuffer((doc) => {
    ...
    if (cn.invoiceId) { y += 12; doc.fillColor(TXT).font('Helvetica-Bold').text('Originating Invoice:', 350, y).font('Helvetica').text((cn as any).invoiceNumber||'', 440, y); }
    y = 220;
...
export async function generateAuditLogsPDF(orgId: string, startDate: Date, endDate: Date): Promise<Buffer> {
  const list = await db.select({
    log: auditLog,
    user: { name: users.fullName, email: users.email }
  ...
  const rows = list.map(row => [formatShortDate(row.log.createdAt), row.log.action?.toUpperCase()||'-', row.log.entityType||'-', (row.user as any)?.name||'-', row.log.ipAddress||'-']);
...
export async function generateIncomeStatementPDF(orgId: string, startDate: Date, endDate: Date): Promise<Buffer> {
  ...
  function writeSection(doc: any, label: string, data: any, y: number) {
  ...
  function writeSubSection(doc: any, label: string, data: any, y: number) {
  ...
  return generatePDFBuffer(async (doc) => {
    ...
    // OPERATING REVENUE
    y = writeSection(doc, 'OPERATING REVENUE', pnl.operatingRevenue, y);

    // OTHER OPERATING INCOME
    if (pnl.otherOperatingIncome?.accounts?.length > 0) {
      y = writeSection(doc, 'OTHER OPERATING INCOME', pnl.otherOperatingIncome, y);
    }
    ...
    // COST OF SALES
    y = writeSection(doc, 'COST OF SALES', pnl.costOfSales, y);
    ...
    y = writeSubSection(doc, 'Staff Costs', pnl.staffCosts, y);
    y = writeSubSection(doc, 'Administrative Expenses', pnl.administrative, y);
    y = writeSubSection(doc, 'Selling & Distribution Expenses', pnl.sellingDistribution, y);
    if (pnl.otherOperatingExpenses?.accounts?.length > 0) {
      y = writeSubSection(doc, 'Other Operating Expenses', pnl.otherOperatingExpenses, y);
    }
    ...
    if (pnl.financeIncome?.accounts?.length > 0) {
      y = writeSection(doc, 'FINANCE INCOME', pnl.financeIncome, y);
    }

    // FINANCE COSTS
    if (pnl.financeCosts?.accounts?.length > 0) {
      y = writeSection(doc, 'FINANCE COSTS', pnl.financeCosts, y);
    }
    ...
    if (pnl.incomeTaxExpense?.accounts?.length > 0) {
      y = writeSection(doc, 'INCOME TAX EXPENSE', pnl.incomeTaxExpense, y);
    }
