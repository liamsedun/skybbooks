    ...
    <div className="w-full flex flex-col space-y-4" id={id || 'data-table-container'}>
      ...
      <div className="bg-white rounded-xl border border-slate-100 overflow-hidden shadow-sm">
          ...
          <table className="w-full text-left border-collapse">
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id} className="border-b border-slate-200">
                  {headerGroup.headers.map((header) => (
                    <th
                      key={header.id}
                      className="px-6 h-12 align-middle text-[11px] font-semibold text-slate-500 uppercase tracking-wider bg-surface-subtle select-none"
                    >
            ...
            <tbody className="divide-y divide-slate-150">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={columns.length} className="px-6 py-12 text-center text-rose-500 bg-rose-50/20">
                    <div className="flex flex-col items-center justify-center space-y-3">
                ...
                table.getRowModel().rows.map((row) => (
                  <tr key={row.id} className="h-12 hover:bg-slate-50/40 transition-colors align-middle">
                    {row.getVisibleCells().map((cell) => (
                      <td key={cell.id} className="px-6 h-12 align-middle text-[13px] text-slate-600 font-medium font-sans">
        ...
        {!isLoading && !error && data.length > pageSize && (
          <div className="px-6 py-3.5 flex items-center justify-between border-t border-slate-100 bg-white">
