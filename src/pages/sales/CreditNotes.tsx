...
function exportCreditNotesCSV(notes: CreditNote[]) {
  const headers = ['CN #','Customer','Date','Status','Subtotal (₦)','Tax (₦)','Total (₦)','Remaining (₦)','Notes'];
...
function exportCreditNotesPDF(notes: CreditNote[]) {
  ...
  <div class="header">
    ...
    <div style="text-align:right"><div class="title">Credit Notes Report</div><div class="date">Generated: ${new Date().toLocaleDateString('en-GB',{day:'2-digit',month:'long',year:'numeric'})}</div><div class="date">${notes.length} credit notes</div></div>
  </div>
  <table><thead><tr><th>CN #</th><th>Customer</th><th>Date</th><th>Status</th><th style="text-align:right">Total</th><th style="text-align:right">Remaining</th></tr></thead><tbody>${rows}</tbody></table>
