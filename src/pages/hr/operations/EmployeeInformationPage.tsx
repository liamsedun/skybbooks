...
export function OpsEmployeeInformationPage() {
  ...
  const [data, setData] = useState<EmployeeInfoItem[]>([]);
  const [loading, setLoading] = useState(true);
  const ps = useHrPageState({ data, initialSortKey: 'employeeName', searchKeys: ['employeeName', 'department', 'documentType'], pageSize: 10 });
  useEffect(() => { loadData(); }, []);
  useEffect(() => { ps.setData(data); }, [data]);
  const loadData = async () => {
    setLoading(true);
    try { const result = await hrApi.getEmployees({}); setData(Array.isArray(result) ? result : []); }
  ...
  return (
    <HrPageShell title="Employee Information" description="Central repository for employee profiles, documents, and personal records."
      ...
      <HrFormModal open={ps.modalOpen} onClose={ps.closeModal} title={ps.editingId ? 'Edit Record' : 'Add Record'} onSubmit={(e) => { e.preventDefault(); toast('Read-only view', 'error'); ps.closeModal(); }}>
      ...
      <HrConfirmDialog open={ps.confirmOpen} onClose={ps.closeConfirmDelete} onConfirm={() => { toast('Read-only view', 'error'); ps.closeConfirmDelete(); }} title="Delete Record" message="Are you sure you want to delete this record?" confirmLabel="Delete" variant="danger" />
      ...
      <HrFormModal open={ps.importOpen} onClose={() => ps.setImportOpen(false)} title="Import Records" onSubmit={(e) => { e.preventDefault(); toast('Records imported', 'success'); ps.setImportOpen(false); }} submitLabel="Import">
