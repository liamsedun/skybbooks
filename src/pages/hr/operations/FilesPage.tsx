...
export function OpsFilesPage() {
  ...
  return (
    <HrPageShell title="Files" description="Organisation and employee document repository with permissions and versioning."
      ...
      <HrFormModal open={ps.modalOpen} onClose={ps.closeModal} title={ps.editingId ? 'Edit File' : 'Upload File'} onSubmit={(e) => { e.preventDefault(); toast('Read-only view', 'error'); ps.closeModal(); }}>
      ...
      <HrFormModal open={ps.importOpen} onClose={() => ps.setImportOpen(false)} title="Import Files" onSubmit={(e) => { e.preventDefault(); toast('Files imported', 'success'); ps.setImportOpen(false); }} submitLabel="Import">
