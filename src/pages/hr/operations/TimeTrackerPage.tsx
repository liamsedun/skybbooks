...
export function OpsTimeTrackerPage() {
  ...
  return (
    <HrPageShell title="Time Tracker" description="Time logs, timesheets, billable hours tracking, and productivity monitoring."
      ...
      <HrFormModal open={ps.modalOpen} onClose={ps.closeModal} title={ps.editingId ? 'Edit Entry' : 'Add Entry'} onSubmit={(e) => { e.preventDefault(); toast('Read-only view', 'error'); ps.closeModal(); }}>
      ...
      <HrConfirmDialog open={ps.confirmOpen} onClose={ps.closeConfirmDelete} onConfirm={() => { toast('Read-only view', 'error'); ps.closeConfirmDelete(); }} title="Delete Entry" message="Are you sure you want to delete this time entry?" confirmLabel="Delete" variant="danger" />
      ...
      <HrFormModal open={ps.importOpen} onClose={() => ps.setImportOpen(false)} title="Import Time Entries" onSubmit={(e) => { e.preventDefault(); toast('Entries imported', 'success'); ps.setImportOpen(false); }} submitLabel="Import">
