...
export function OpsShiftPage() {
  ...
  return (
    <HrPageShell title="Shift" description="Shift pattern creation, roster assignments, and shift swap management."
      ...
      <HrFormModal open={ps.modalOpen} onClose={ps.closeModal} title={ps.editingId ? 'Edit Shift' : 'Add Shift'} onSubmit={(e) => { e.preventDefault(); toast('Read-only view', 'error'); ps.closeModal(); }}>
      ...
      <HrConfirmDialog open={ps.confirmOpen} onClose={ps.closeConfirmDelete} onConfirm={() => { toast('Read-only view', 'error'); ps.closeConfirmDelete(); }} title="Delete Shift" message="Are you sure you want to delete this shift?" confirmLabel="Delete" variant="danger" />
      ...
      <HrFormModal open={ps.importOpen} onClose={() => ps.setImportOpen(false)} title="Import Shifts" onSubmit={(e) => { e.preventDefault(); toast('Shifts imported', 'success'); ps.setImportOpen(false); }} submitLabel="Import">
