...
const ALL_TABS: TabItem[] = [
  { label: 'Time Logs', path: '/app/hr/timesheets/logs', icon: Timer },
  { label: 'Timesheets', path: '/app/hr/timesheets/sheets', icon: FileText },
];

export function TimesheetsLayout() {
  ...
  if (TABS.length === 0) {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-xl font-bold text-ink-900">Time Tracker</h1>
          <p className="text-sm text-ink-400 mt-0.5">Track time logs and manage employee timesheets.</p>
        </div>
        <div className="bg-surface rounded-2xl border border-border-custom shadow-sm p-8 text-center text-ink-400">
          <Timer className="w-12 h-12 mx-auto mb-3 text-ink-300" />
