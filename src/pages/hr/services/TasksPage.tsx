...
export function TasksPage() {
  ...
  return (
    <HrPageShell title="Tasks" description="Task assignments, workflows, approvals, and action item tracking"
      ...
      headerActions={
        <>
          <button onClick={() => { exportToCsv(['Task', 'Assigned To', 'Priority', 'Due Date', 'Status'], ps.filtered.map(i => [i.title, i.assignedTo, i.priority, i.dueDate, i.status]), 'hr-tasks'); showSuccess('CSV exported'); }} className="inline-flex items-center gap-1.5 px-3 py-1.5 border border-border-custom text-ink-600 text-xs font-medium rounded-xl hover:bg-ink-50 dark:hover:bg-ink-800 transition-all"><Download className="w-3.5 h-3.5" /> CSV</button>
