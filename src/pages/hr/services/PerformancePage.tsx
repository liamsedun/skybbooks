...
export function PerformancePage() {
  ...
  return (
    <HrPageShell title="Performance" description="Performance reviews, appraisals, goal tracking, and feedback management"
      ...
      headerActions={
        <>
          <button onClick={() => { exportToCsv(['Employee', 'Reviewer', 'Period', 'Rating', 'Status'], ps.filtered.map(i => [i.employeeName, i.reviewer, i.period, String(i.rating), i.status]), 'performance-reviews'); showSuccess('CSV exported'); }} className="inline-flex items-center gap-1.5 px-3 py-1.5 border border-border-custom text-ink-600 text-xs font-medium rounded-xl hover:bg-ink-50 dark:hover:bg-ink-800 transition-all"><Download className="w-3.5 h-3.5" /> CSV</button>
