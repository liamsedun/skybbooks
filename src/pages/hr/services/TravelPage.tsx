...
export function TravelPage() {
  ...
  const columns: Column<TravelRequest>[] = [
    ...
    { key: 'amount', label: 'Amount', sortable: true, render: (i) => <span className="text-ink-600 font-medium">₦{i.amount.toLocaleString()}</span> },
  ...
  return (
    <HrPageShell title="Travel" description="Travel requests, approvals, expense reporting, and itinerary management"
      ...
      headerActions={
        <>
          <button onClick={() => { exportToCsv(['Employee', 'Destination', 'From', 'To', 'Amount', 'Purpose', 'Status'], ps.filtered.map(i => [i.employeeName, i.destination, i.fromDate, i.toDate, String(i.amount), i.purpose, i.status]), 'travel-requests'); toast('CSV exported', 'success'); }} className="inline-flex items-center gap-1.5 px-3 py-1.5 border border-border-custom text-ink-600 text-xs font-medium rounded-xl hover:bg-ink-50 dark:hover:bg-ink-800 transition-all"><Download className="w-3.5 h-3.5" /> CSV</button>
      ...
      <HrFormModal open={ps.modalOpen} onClose={ps.closeModal} title={ps.editingId ? 'Edit Travel Request' : 'Add Travel Request'} onSubmit={(e) => { e.preventDefault(); toast('Read-only view', 'error'); ps.closeModal(); }}>
        ...
        <div><label className="block text-xs font-medium text-ink-500 mb-1">Estimated Amount (₦)</label><input type="number" className="w-full px-3 py-2.5 text-sm border border-border-custom rounded-xl bg-surface text-ink-900 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all" /></div>
      </HrFormModal>
      <HrConfirmDialog open={ps.confirmOpen} onClose={ps.closeConfirmDelete} onConfirm={() => { toast('Read-only view', 'error'); ps.closeConfirmDelete(); }} title="Delete Request" message="Are you sure you want to delete this travel request? This action cannot be undone." confirmLabel="Delete" variant="danger" />
          ...
          return (
            <div className="space-y-4 text-sm">
              <div className="grid grid-cols-2 gap-4 p-4 bg-ink-50 dark:bg-ink-800/50 rounded-xl">
                ...
                <div><span className="text-ink-400 text-xs block">Amount</span><span className="font-medium text-ink-900">₦{item.amount.toLocaleString()}</span></div>
      ...
      <HrFormModal open={ps.importOpen} onClose={() => ps.setImportOpen(false)} title="Import Travel Requests" onSubmit={(e) => { e.preventDefault(); toast('Read-only view', 'error'); ps.setImportOpen(false); }} submitLabel="Import">
