...
import { CalendarDays, CheckCircle2, Clock, XCircle, Plus, Download, Upload, FileText, Search, Edit3, Trash2, Eye } from 'lucide-react';
...
const MOCK: LeaveRecord[] = [
  { id: 'L001', employeeName: 'Amara Okafor', leaveType: 'Annual', fromDate: '2026-08-01', toDate: '2026-08-12', days: 10, status: 'approved', reason: 'Family vacation' },
  { id: 'L002', employeeName: 'Chidi Nwosu', leaveType: 'Sick', fromDate: '2026-07-20', toDate: '2026-07-21', days: 2, status: 'approved', reason: 'Malaria' },
  { id: 'L003', employeeName: 'Fatima Usman', leaveType: 'Maternity', fromDate: '2026-06-01', toDate: '2026-08-31', days: 90, status: 'approved', reason: 'Maternity leave' },
  { id: 'L004', employeeName: 'Emeka Eze', leaveType: 'Annual', fromDate: '2026-09-05', toDate: '2026-09-09', days: 5, status: 'pending', reason: 'Personal trip' },
  { id: 'L005', employeeName: 'Yetunde Bello', leaveType: 'Sick', fromDate: '2026-07-28', toDate: '2026-07-28', days: 1, status: 'rejected', reason: 'Doctor appointment' },
  { id: 'L006', employeeName: 'Segun Adeyemi', leaveType: 'Paternity', fromDate: '2026-08-10', toDate: '2026-08-17', days: 7, status: 'pending', reason: 'New baby' },
  { id: 'L007', employeeName: 'Ngozi Obi', leaveType: 'Annual', fromDate: '2026-10-01', toDate: '2026-10-08', days: 6, status: 'pending', reason: 'Travel abroad' },
  { id: 'L008', employeeName: 'Ibrahim Danjuma', leaveType: 'Compassionate', fromDate: '2026-07-15', toDate: '2026-07-16', days: 2, status: 'approved', reason: 'Family event' },
];
...
export function LeaveSummaryPage() {
  ...
  const stats = useMemo(() => [
    { label: 'Total Requests', value: MOCK.length, icon: <CalendarDays className="w-4 h-4" />, color: 'blue' as const, active: ps.statusFilter === 'all', onClick: () => ps.setStatusFilter('all') },
    { label: 'Approved', value: MOCK.filter(i => i.status === 'approved').length, icon: <CheckCircle2 className="w-4 h-4" />, color: 'green' as const, active: ps.statusFilter === 'approved', onClick: () => ps.setStatusFilter('approved') },
    ...
    { label: 'Rejected', value: MOCK.filter(i => i.status === 'rejected').length, icon: <XCircle className="w-4 h-4" />, color: 'red' as const, active: ps.statusFilter === 'rejected', onClick: () => ps.setStatusFilter('rejected') },
  ], [ps.statusFilter]);
    ...
    <HrPageShell title="Leave Summary" description="Overview of all leave requests across the organisation"
      ...
      <HrFormModal open={ps.addModalOpen || ps.editModalOpen} onClose={ps.closeModals} title={ps.editModalOpen ? 'Edit Leave Request' : 'New Leave Request'}>
        <div className="space-y-4">
          ...
          <div><label className="block text-xs font-medium text-ink-600 mb-1">Leave Type</label><input className="w-full h-9 px-3 text-sm rounded-lg border border-ink-200 dark:border-ink-700 bg-white dark:bg-ink-900 text-ink-900" defaultValue={editItem?.leaveType ?? ''} /></div>
          ...
          <button className="w-full h-9 text-sm font-medium text-white bg-primary hover:bg-primary-hover rounded-lg transition-colors" onClick={() => { success(ps.editModalId ? 'Leave request updated' : 'Leave request created'); ps.closeModals(); }}>{ps.editModalId ? 'Update' : 'Submit'}</button>
        </div>
      </HrFormModal>
