...
const MOCK: WorkflowItem[] = [
  { id: '1', name: 'Employee Onboarding', module: 'People', steps: 8, version: 3, status: 'active' },
  { id: '2', name: 'Leave Approval', module: 'Time & Attendance', steps: 4, version: 2, status: 'active' },
  { id: '3', name: 'Travel Request', module: 'Travel', steps: 5, version: 1, status: 'active' },
  { id: '4', name: 'Expense Reimbursement', module: 'Finance', steps: 6, version: 2, status: 'active' },
  { id: '5', name: 'Performance Review', module: 'Performance', steps: 7, version: 1, status: 'draft' },
  { id: '6', name: 'Offboarding Process', module: 'People', steps: 10, version: 1, status: 'draft' },
  { id: '7', name: 'Purchase Request', module: 'Operations', steps: 4, version: 2, status: 'archived' },
  { id: '8', name: 'Compensatory Off', module: 'Time & Attendance', steps: 3, version: 1, status: 'active' },
];

export function WorkflowsPage() {
  const { success: showSuccess } = useToast();
  const ps = useHrPageState({ data: MOCK, initialSortKey: 'name', searchKeys: ['name', 'module'], pageSize: 10 });

  const stats = useMemo(() => [
    { label: 'Total Templates', value: MOCK.length, icon: <GitBranch className="w-4 h-4" />, color: 'blue' as const, active: ps.statusFilter === 'all', onClick: () => ps.setStatusFilter('all') },
    { label: 'Active', value: MOCK.filter(i => i.status === 'active').length, icon: <Activity className="w-4 h-4" />, color: 'emerald' as const, active: ps.statusFilter === 'active', onClick: () => ps.setStatusFilter('active') },
    { label: 'Draft', value: MOCK.filter(i => i.status === 'draft').length, icon: <FileText className="w-4 h-4" />, color: 'amber' as const, active: ps.statusFilter === 'draft', onClick: () => ps.setStatusFilter('draft') },
    { label: 'Archived', value: MOCK.filter(i => i.status === 'archived').length, icon: <Layers className="w-4 h-4" />, color: 'slate' as const, active: ps.statusFilter === 'archived', onClick: () => ps.setStatusFilter('archived') },
  ], [ps.statusFilter]);
  ...
  return (
    <HrPageShell title="Workflow Templates" description="Manage HR workflow templates"
      ...
      headerActions={
        <>
          <button onClick={() => { exportToCsv(['Name', 'Module', 'Steps', 'Version', 'Status'], MOCK.map(w => [w.name, w.module, String(w.steps), String(w.version), w.status]), 'workflows'); showSuccess('Exported'); }} className="inline-flex items-center gap-1.5 px-3 py-1.5 border border-border-custom text-ink-600 text-xs font-medium rounded-xl hover:bg-ink-50 dark:hover:bg-ink-800 transition-all"><Download className="w-3.5 h-3.5" /> CSV</button>
          <button onClick={() => exportToPdf('Workflow Templates', ['Name', 'Module', 'Steps', 'Status'], MOCK.map(w => [w.name, w.module, String(w.steps), w.status]), 'workflows')} className="inline-flex items-center gap-1.5 px-3 py-1.5 border border-border-custom text-ink-600 text-xs font-medium rounded-xl hover:bg-ink-50 dark:hover:bg-ink-800 transition-all"><FileText className="w-3.5 h-3.5" /> PDF</button>
      ...
      <HrFormModal open={ps.modalOpen} onClose={ps.closeModal} title={ps.editingId ? 'Edit Template' : 'Create Template'} onSubmit={(e) => { e.preventDefault(); showSuccess(ps.editingId ? 'Updated' : 'Created'); ps.closeModal(); }}>
      ...
      <HrConfirmDialog open={ps.confirmOpen} onClose={ps.closeConfirmDelete} onConfirm={() => { showSuccess('Deleted'); ps.closeConfirmDelete(); }} title="Delete Template" message="Are you sure?" confirmLabel="Delete" variant="danger" />
      <HrViewDrawer open={ps.viewDrawerOpen} onClose={ps.closeViewDrawer} title="Workflow Details">
        {ps.viewingId && (() => { const w = MOCK.find(i => i.id === ps.viewingId)!; return (
