...
export function OffboardingPage() {
  ...
  return (
    <HrPageShell title="Offboarding" description="Manage employee offboarding"
      ...
      headerActions={
        <>
          <button onClick={() => { exportToCsv(['Employee', 'Department', 'Exit Date', 'Reason', 'Status'], data.map(o => [o.employeeName, o.department, o.exitDate, o.reason, o.status]), 'offboarding'); toast('Exported', 'success'); }} className="inline-flex items-center gap-1.5 px-3 py-1.5 border border-border-custom text-ink-600 text-xs font-medium rounded-xl hover:bg-ink-50 dark:hover:bg-ink-800 transition-all"><Download className="w-3.5 h-3.5" /> CSV</button>
      ...
      <HrFormModal open={ps.modalOpen} onClose={ps.closeModal} title={ps.editingId ? 'Edit Offboarding' : 'Start Offboarding'} onSubmit={(e) => { e.preventDefault(); toast('Read-only view', 'error'); ps.closeModal(); }}>
