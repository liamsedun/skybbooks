...
const MOCK: Recognition[] = [
  { id: 'rec-1', fromEmployee: 'Alice Johnson', toEmployee: 'Bob Smith', type: 'award', message: 'Outstanding performance this quarter!', date: '2026-08-01', status: 'published' },
  { id: 'rec-2', fromEmployee: 'Carol White', toEmployee: 'David Lee', type: 'shoutout', message: 'Great teamwork on the project.', date: '2026-08-03', status: 'published' },
  { id: 'rec-3', fromEmployee: 'Eve Brown', toEmployee: 'Frank Wilson', type: 'anniversary', message: '5 amazing years with us!', date: '2026-08-05', status: 'published' },
  { id: 'rec-4', fromEmployee: 'Grace Kim', toEmployee: 'Henry Davis', type: 'shoutout', message: 'Thanks for covering my shift.', date: '2026-08-07', status: 'draft' },
  { id: 'rec-5', fromEmployee: 'Ivy Chen', toEmployee: 'Jack Taylor', type: 'award', message: 'Employee of the Month - July', date: '2026-08-01', status: 'published' },
  { id: 'rec-6', fromEmployee: 'Kevin Moore', toEmployee: 'Laura Garcia', type: 'anniversary', message: '1 year work anniversary!', date: '2026-08-10', status: 'published' },
  { id: 'rec-7', fromEmployee: 'Mike Martinez', toEmployee: 'Nina Rodriguez', type: 'shoutout', message: 'Fantastic presentation skills.', date: '2026-08-12', status: 'draft' },
  { id: 'rec-8', fromEmployee: 'Olivia Anderson', toEmployee: 'Paul Thomas', type: 'award', message: 'Innovation Award Q3 2026', date: '2026-08-15', status: 'published' },
];
...
export function RecognitionPage() {
  ...
  const ps = useHrPageState({ data: MOCK, initialSortKey: 'fromEmployee', searchKeys: ['fromEmployee', 'toEmployee', 'message'], pageSize: 10 });
  const stats = useMemo(() => [
    ...
    { label: 'Published', value: MOCK.filter(i => i.status === 'published').length, icon: <Sparkles className="w-4 h-4" />, color: 'emerald' as const, active: ps.statusFilter === 'published', onClick: () => ps.setStatusFilter('published') },
    { label: 'Draft', value: MOCK.filter(i => i.status === 'draft').length, icon: <Heart className="w-4 h-4" />, color: 'amber' as const, active: ps.statusFilter === 'draft', onClick: () => ps.setStatusFilter('draft') },
  ], [ps.statusFilter]);
  ...
  return (
    <HrPageShell title="Recognition" description="Recognize and celebrate employee achievements"
      ...
      <HrFormModal open={ps.modalOpen} onClose={ps.closeModal} title={ps.editingId ? 'Edit Recognition' : 'Add Recognition'} onSubmit={(e) => { e.preventDefault(); showSuccess(ps.editingId ? 'Updated' : 'Created'); ps.closeModal(); }}>
        ...
        <div><label className="block text-xs font-medium text-ink-500 mb-1">Type</label><select className="w-full px-3 py-2.5 text-sm border border-border-custom rounded-xl bg-surface text-ink-900 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"><option value="award">Award</option><option value="shoutout">Shoutout</option><option value="anniversary">Anniversary</option></select></div>
        <div><label className="block text-xs font-medium text-ink-500 mb-1">Message</label><textarea className="w-full px-3 py-2.5 text-sm border border-border-custom rounded-xl bg-surface text-ink-900 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all" rows={3} placeholder="Write a recognition message..." /></div>
      ...
      <HrConfirmDialog open={ps.confirmOpen} onClose={ps.closeConfirmDelete} onConfirm={() => { showSuccess('Deleted'); ps.closeConfirmDelete(); }} title="Delete Recognition" message="Are you sure you want to delete this recognition?" confirmLabel="Delete" variant="danger" />
