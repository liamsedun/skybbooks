...
export function LateCheckOutPage() {
  ...
  return (
    <HrPageShell title="Late Check Out" description="Late checkout records, overtime patterns, and after-hours work trends"
      pageKey="reports"
      headerActions={<>
        <button onClick={() => { exportToCsv(csvHeaders, csvRows, 'late-checkout'); success('CSV exported'); }} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-ink-600 bg-ink-100 dark:bg-ink-800 hover:bg-ink-200 dark:hover:bg-ink-700 rounded-lg transition-colors"><Download className="w-3.5 h-3.5" /> CSV</button>
        <button onClick={() => exportToPdf('Late Check Out', pdfHeaders, pdfRows, 'late-checkout')} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-ink-600 bg-ink-100 dark:bg-ink-800 hover:bg-ink-200 dark:hover:bg-ink-700 rounded-lg transition-colors"><FileText className="w-3.5 h-3.5" /> PDF</button>
      ...
      <HrFilterBar
        ...
        onExportPdf={() => exportToPdf('Late Check Out', pdfHeaders, pdfRows, 'late-checkout')}
      />
      <HrDataTable columns={columns} data={paginated} keyExtractor={i => i.id}
        ...
        emptyMessage="No late check-out records found" emptyAction={<button onClick={ps.openAddModal} className="text-xs font-medium text-primary">Add record</button>} />
      <HrFormModal open={ps.modalOpen} onClose={ps.closeModal} title={ps.editingId ? 'Edit Late Check-Out' : 'New Late Check-Out'}>
      ...
      <HrViewDrawer open={ps.viewDrawerOpen} onClose={ps.closeViewDrawer} title="Late Check-Out Details">
