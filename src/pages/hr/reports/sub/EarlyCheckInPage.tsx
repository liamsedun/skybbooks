...
export function EarlyCheckInPage() {
  ...
  const stats = useMemo(() => [
    { label: 'Total Records', value: data.length, icon: <LogIn className="w-4 h-4" />, color: 'blue' as const, active: ps.statusFilter === 'all', onClick: () => ps.setStatusFilter('all') },
    { label: 'Early', value: data.filter((i: any) => i.status === 'early').length, icon: <Sun className="w-4 h-4" />, color: 'emerald' as const, active: ps.statusFilter === 'early', onClick: () => ps.setStatusFilter('early') },
    { label: 'On Time', value: data.filter((i: any) => i.status === 'on_time').length, icon: <CheckCircle2 className="w-4 h-4" />, color: 'blue' as const, active: ps.statusFilter === 'on_time', onClick: () => ps.setStatusFilter('on_time') },
  ...
  return (
    <HrPageShell title="Early Check In" description="Employees who checked in early, with frequency and trend analysis"
      pageKey="reports"
      headerActions={<>
        <button onClick={() => { exportToCsv(csvHeaders, csvRows, 'early-checkin'); toast('CSV exported', 'success'); }} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-ink-600 bg-ink-100 dark:bg-ink-800 hover:bg-ink-200 dark:hover:bg-ink-700 rounded-lg transition-colors"><Download className="w-3.5 h-3.5" /> CSV</button>
