...
export function EarlyCheckOutPage() {
  ...
  const columns: Column<EarlyCheckOut>[] = [
    ...
    { key: 'minutesEarly', label: 'Early (min)', sortable: true, render: (i) => <span className="font-medium text-amber-600">{i.minutesEarly} min</span>, className: 'text-center' },
  ...
  return (
    <HrPageShell title="Early Check Out" description="Early departure records, frequency analysis, and department comparisons"
      ...
      <HrFilterBar
        ...
        onExportPdf={() => exportToPdf('Early Check Out', pdfHeaders, pdfRows, 'early-checkout')}
      />
      <HrDataTable columns={columns} data={paginated} keyExtractor={i => i.id}
        ...
        emptyMessage="No early check-out records found" emptyAction={<button onClick={ps.openAddModal} className="text-xs font-medium text-primary">Add record</button>} />
      <HrFormModal open={ps.modalOpen} onClose={ps.closeModal} title={ps.editingId ? 'Edit Early Check-Out' : 'New Early Check-Out'}>
      ...
      <HrViewDrawer open={ps.viewDrawerOpen} onClose={ps.closeViewDrawer} title="Early Check-Out Details">
        {selectedItem && <div className="space-y-3">
          ...
          <div><label className="text-xs text-ink-500">Minutes Early</label><p className="text-sm font-medium text-amber-600">{selectedItem.minutesEarly} min</p></div>
