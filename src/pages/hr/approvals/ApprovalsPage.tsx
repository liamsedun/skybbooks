...
export function ApprovalsPage() {
  ...
  const columns: Column<ApprovalItem>[] = [
    ...
    { key: 'actions', label: '', render: (i) => (
      <div className="flex items-center justify-end gap-1" onClick={e => e.stopPropagation()}>
        {i.status === 'pending' && (<>
          <button onClick={() => showError('Read-only view')} className="w-7 h-7 flex items-center justify-center rounded-lg text-emerald-500 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-950/30 transition-colors" title="Approve"><Check className="w-3.5 h-3.5" /></button>
          <button onClick={() => showError('Read-only view')} className="w-7 h-7 flex items-center justify-center rounded-lg text-rose-500 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/30 transition-colors" title="Reject"><X className="w-3.5 h-3.5" /></button>
        </>)}
  ...
  return (
    <HrPageShell title="Approvals" description="Pending approval requests"
      ...
      <HrFormModal open={ps.modalOpen} onClose={ps.closeModal} title={ps.editingId ? 'Edit Request' : 'New Request'} onSubmit={(e) => { e.preventDefault(); showError('Read-only view'); ps.closeModal(); }}>
