  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-ink-900">Enterprise Contracts</h1>
          <p className="text-sm text-ink-500 mt-1">Manage negotiated enterprise agreements</p>
        </div>
        ...
        {isLoading ? (
          ...
          <table className="w-full text-sm">
            <thead><tr className="border-b bg-surface-subtle">
              <th className="text-left p-3 font-medium text-ink-600">Contract</th>
              <th className="text-left p-3 font-medium text-ink-600">Org</th>
              <th className="text-left p-3 font-medium text-ink-600">Plan</th>
              <th className="text-right p-3 font-medium text-ink-600">Value</th>
              <th className="text-center p-3 font-medium text-ink-600">Status</th>
              <th className="text-center p-3 font-medium text-ink-600">Dates</th>
            ...
            <tbody>
              {data?.map((item: any) => (
                <tr key={item.id} className="border-b hover:bg-surface-hover">
                  <td className="p-3">
                    ...
                    <div className="text-xs text-ink-400">{item.contractNumber}</div>
                  </td>
                  <td className="p-3 text-ink-600">{item.orgName || item.orgId?.slice(0,8)}</td>
                  <td className="p-3 text-ink-600">{item.planName || '-'}</td>
                  ...
                  <td className="p-3 text-center text-xs text-ink-500">
              ...
              {(!data || data.length === 0) && <tr><td colSpan={7} className="p-6 text-center text-ink-400">No enterprise contracts</td></tr>}
            </tbody>
          </table>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setEditing(null)}>
          <div className="bg-surface rounded-xl p-6 w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-ink-500 mb-1">Contract Name *</label>
                <input value={editing.name || ''} onChange={e => setEditing({ ...editing, name: e.target.value })}
                  className="w-full border rounded-lg px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-xs text-ink-500 mb-1">Contract Number *</label>
              ...
              <div>
                <label className="block text-xs text-ink-500 mb-1">Negotiated Price (kobo)</label>
