  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-ink-900">Regional Pricing</h1>
          <p className="text-sm text-ink-500 mt-1">Manage plan pricing per region/currency</p>
        </div>
        ...
        {isLoading ? (
          ...
          <table className="w-full text-sm">
            <thead><tr className="border-b bg-surface-subtle">
              ...
              <th className="text-left p-3 font-medium text-ink-600">Region</th>
              <th className="text-left p-3 font-medium text-ink-600">Currency</th>
              <th className="text-right p-3 font-medium text-ink-600">Monthly</th>
              <th className="text-right p-3 font-medium text-ink-600">Annual</th>
            ...
            <tbody>
              {data?.map((item: any) => (
                <tr key={item.id} className="border-b hover:bg-surface-hover">
                  <td className="p-3 font-medium text-ink-900">{item.planName || item.planId?.slice(0,8)}</td>
                  <td className="p-3"><span className="uppercase font-mono text-xs bg-surface-hover px-2 py-0.5 rounded">{item.region}</span></td>
                  <td className="p-3 text-ink-600">{item.currency}</td>
              ...
              {(!data || data.length === 0) && <tr><td colSpan={7} className="p-6 text-center text-ink-400">No regional pricing entries</td></tr>}
            </tbody>
          </table>
        )}
      </div>
          ...
          <div className="bg-surface rounded-xl p-6 w-full max-w-lg mx-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-ink-500 mb-1">Monthly Price (kobo)</label>
                  <input type="number" value={editing.monthlyPriceKobo || 0} onChange={e => setEditing({ ...editing, monthlyPriceKobo: Number(e.target.value) })}
                    className="w-full border rounded-lg px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="block text-xs text-ink-500 mb-1">Annual Price (kobo)</label>
