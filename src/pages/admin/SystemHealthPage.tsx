  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => window.location.reload()} className="flex items-center gap-2 px-3 py-2 text-sm bg-surface border rounded-lg hover:bg-surface-hover">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-surface rounded-xl border p-5">
          <h3 className="text-sm font-semibold text-ink-700 mb-4">Request Volume (24h)</h3>
        ...
        <div className="bg-surface rounded-xl border p-5">
          <h3 className="text-sm font-semibold text-ink-700 mb-4">Error Rate (24h)</h3>
          <ResponsiveContainer width="100%" height={250}>
            <AreaChart data={MOCK_REQUEST_VOLUME}>
              <defs>
      ...
      <div className="bg-surface rounded-xl border">
        <div className="px-5 py-4 border-b">
          <h3 className="text-sm font-semibold text-ink-700">Recent Errors</h3>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-surface-subtle">
              <th className="text-left p-3 font-medium text-ink-600">Time</th>
              <th className="text-left p-3 font-medium text-ink-600">Route</th>
              ...
              <th className="text-left p-3 font-medium text-ink-600">Count</th>
            </tr>
          </thead>
          <tbody>
            {MOCK_ERRORS.map((e, i) => (
              <tr key={i} className="border-b hover:bg-surface-hover">
                <td className="p-3 text-ink-500">{e.time}</td>
                <td className="p-3 font-mono text-xs text-ink-900">{e.route}</td>
                <td className="p-3 text-ink-700">{e.message}</td>
                <td className="p-3">
                  <span className={`px-2 py-0.5 text-xs rounded-full ${e.status >= 500 ? 'bg-red-100 text-red-700' : e.status === 429 ? 'bg-amber-100 text-amber-700' : 'bg-surface-hover text-ink-600'}`}>
                    {e.status}
                  </span>
                </td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="bg-surface rounded-xl border p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-ink-500">Application Server</p>
            <p className="text-sm font-semibold text-ink-900">{healthData?.appServer || 'Healthy'}</p>
          </div>
        </div>
        <div className="bg-surface rounded-xl border p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm font-semibold text-ink-900">{healthData?.dbStatus || 'Connected'}</p>
          </div>
        </div>
        <div className="bg-surface rounded-xl border p-4 flex items-center gap-3">
          <div className="p-2 rounded-lg bg-purple-50 text-purple-600"><Zap className="w-5 h-5" /></div>
          <div>
            <p className="text-xs text-ink-500">Cache / Queue</p>
            <p className="text-sm font-semibold text-ink-900">{healthData?.cacheStatus || 'Operational'}</p>
          </div>
        </div>
      </div>
    </div>
