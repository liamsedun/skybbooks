    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Platform-wide audit trail across all organizations</p>
        </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          ...
          {showFilter && (
            <div className="absolute top-full mt-1 right-0 bg-white border rounded-lg shadow-lg z-10 w-36">
              {ACTIONS.map(a => (
                <button key={a} onClick={() => { setActionFilter(a); setPage(1); setShowFilter(false); }}
                  className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-50 ${actionFilter === a ? 'bg-blue-50 text-blue-600' : 'text-gray-700'}`}>
                  {a ? a : 'All Actions'}
                </button>
              ))}
            </div>
      ...
      <div className="bg-white rounded-xl border">
        <table className="w-full text-sm">
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr><td colSpan={6} className="p-12 text-center text-gray-400">No audit log entries found</td></tr>
            ) : (
              data.data.map((entry) => (
                <tr key={entry.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-3">
                    <span className="flex items-center gap-1.5">
                      ...
                      <span className="text-gray-700">{entry.userName || 'System'}</span>
                    </span>
                  </td>
                  ...
                  <td className="p-3">
                    <span className={`px-2 py-0.5 text-xs rounded-full font-medium ${ACTION_STYLES[entry.action] || 'bg-gray-100 text-gray-600'}`}>
                      {entry.action}
                    </span>
                  </td>
                  <td className="p-3">
                    <span className="text-xs font-mono bg-gray-100 px-1.5 py-0.5 rounded">{entry.entityType}</span>
                    <span className="text-xs text-gray-400 ml-1">#{entry.entityId?.slice(0, 8)}</span>
                  </td>
                  <td className="p-3 text-gray-600 text-xs max-w-xs truncate">{entry.description || '—'}</td>
                </tr>
              ))
            )}
          </tbody>
        ...
        {data && data.total > 30 && (
          <div className="px-3 py-3 border-t flex items-center justify-between">
            <p className="text-xs text-gray-500">Showing {(page - 1) * 30 + 1}–{Math.min(page * 30, data.total)} of {data.total}</p>
