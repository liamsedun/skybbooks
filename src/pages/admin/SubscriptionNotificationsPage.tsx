  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Subscription Notifications</h1>
          <p className="text-sm text-gray-500 mt-1">Manage notification templates, view history, and configure preferences</p>
        </div>
      </div>
      ...
      {tab === 'templates' && (
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <p className="text-sm text-gray-500">Customize notification templates per event type and channel. Leave empty to use defaults.</p>
                ...
                <tbody>
                  {templates?.map((t: any) => (
                    <tr key={t.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-3 text-gray-600 max-w-[200px] truncate">{t.subject}</td>
                      <td className="p-3">{t.isActive ? <CheckCircle2 className="w-4 h-4 text-green-500" /> : <X className="w-4 h-4 text-gray-300" />}</td>
                      <td className="p-3 text-right">
                        <button onClick={() => setEditingTemplate(t)} className="p-1 hover:bg-gray-100 rounded mr-1"><Pen className="w-4 h-4 text-gray-500" /></button>
                  ...
                  {(!templates || templates.length === 0) && (
                    <tr><td colSpan={5} className="p-6 text-center text-gray-400">No custom templates yet. Defaults will be used.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
            ...
            <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setEditingTemplate(null)}>
              <div className="bg-white rounded-xl p-6 w-full max-w-lg mx-4" onClick={e => e.stopPropagation()}>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-xs text-gray-500 mb-1">Subject (email only)</label>
      ...
      {tab === 'log' && (
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-gray-500">{logData?.total || 0} entries</span>
          </div>
          <div className="bg-white rounded-xl border overflow-hidden">
            <table className="w-full text-sm">
              <thead>
              ...
              <tbody>
                {logData?.data?.map((entry: any) => (
                  <tr key={entry.id} className="border-b hover:bg-gray-50">
                    <td className="p-3 text-gray-500 text-xs">{fmtDate(entry.createdAt)}</td>
                    ...
                    <td className="p-3 text-gray-600">{entry.recipient || '-'}</td>
                    <td className="p-3 text-gray-600 max-w-[200px] truncate">{entry.subject || '-'}</td>
                    <td className="p-3">
        ...
        <div className="max-w-2xl space-y-6">
          <div className="bg-white rounded-xl border p-5">
            ...
            <div className="space-y-3">
              {CHANNELS.map(ch => (
                <label key={ch.id} className="flex items-center gap-3 p-3 border rounded-lg hover:bg-gray-50 cursor-pointer">
                  ...
                  <ch.icon className="w-5 h-5 text-gray-500" />
                  <div>
                    <p className="text-sm font-medium">{ch.label}</p>
                    <p className="text-xs text-gray-400">{ch.id === 'sms' || ch.id === 'whatsapp' ? 'Future-ready — not yet active' : 'Active'}</p>
                  </div>
                </label>
              ))}
            </div>
          ...
          <div className="bg-white rounded-xl border p-5">
            <h3 className="text-sm font-semibold text-gray-700 mb-4">Enable/Disable Events</h3>
            <div className="grid grid-cols-2 gap-2">
              {EVENT_TYPES.map(et => (
                <label key={et.id} className="flex items-center gap-2 p-2 border rounded hover:bg-gray-50 cursor-pointer text-sm">
