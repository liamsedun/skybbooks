  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-ink-900">Org Configuration</h1>
          <p className="text-sm text-ink-500 mt-1">Manage key-value configuration per organization</p>
        </div>
        ...
        {isLoading ? (
          ...
          <table className="w-full text-sm">
            <thead><tr className="border-b bg-surface-subtle">
              ...
              <th className="text-left p-3 font-medium text-ink-600">Key</th>
              <th className="text-left p-3 font-medium text-ink-600">Value</th>
              <th className="text-left p-3 font-medium text-ink-600">Description</th>
              <th className="text-right p-3 font-medium text-ink-600">Updated</th>
            ...
            <tbody>
              {data?.map((item: any) => (
                <tr key={item.id} className="border-b hover:bg-surface-hover">
                  ...
                  <td className="p-3"><span className="font-mono text-xs bg-surface-hover px-2 py-0.5 rounded">{item.key}</span></td>
                  <td className="p-3 max-w-[200px] truncate font-mono text-xs text-ink-800">
                    {typeof item.value === 'object' ? JSON.stringify(item.value) : String(item.value)}
                  </td>
                  <td className="p-3 text-ink-500 text-xs">{item.description || '-'}</td>
                  <td className="p-3 text-right text-xs text-ink-400">{fmtDate(item.updatedAt || item.createdAt)}</td>
              ...
              {(!data || data.length === 0) && <tr><td colSpan={6} className="p-6 text-center text-ink-400">No config entries</td></tr>}
            </tbody>
          </table>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setEditing(null)}>
          <div className="bg-surface rounded-xl p-6 w-full max-w-lg mx-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs text-ink-500 mb-1">Key *</label>
              ...
              <div>
                ...
                <input value={editing.value ?? ''} onChange={e => setEditing({ ...editing, value: e.target.value })}
                  ...
                  className="w-full border rounded-lg px-3 py-2 text-sm" />
                <p className="text-xs text-ink-400 mt-1">For booleans/numbers enter raw value; for objects enter JSON string</p>
              </div>
              <div>
