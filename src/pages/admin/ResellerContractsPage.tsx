  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Reseller Contracts</h1>
          <p className="text-sm text-gray-500 mt-1">Manage reseller partnership agreements</p>
        </div>
        ...
        {isLoading ? (
          ...
          <table className="w-full text-sm">
            <thead><tr className="border-b bg-gray-50">
              <th className="text-left p-3 font-medium text-gray-600">Reseller</th>
            ...
            <tbody>
              {data?.map((item: any) => (
                <tr key={item.id} className="border-b hover:bg-gray-50">
                  <td className="p-3">
                    <div className="font-medium text-gray-900">{item.resellerName}</div>
                    <div className="text-xs text-gray-400">{item.contactEmail || '-'}</div>
                  </td>
                  <td className="p-3"><span className="font-mono text-xs bg-gray-100 px-2 py-0.5 rounded">{item.resellerCode}</span></td>
                  <td className="p-3 text-gray-600">{item.resellerOrgName || item.resellerOrgId?.slice(0,8)}</td>
              ...
              {(!data || data.length === 0) && <tr><td colSpan={7} className="p-6 text-center text-gray-400">No reseller contracts</td></tr>}
            </tbody>
          </table>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setEditing(null)}>
          <div className="bg-white rounded-xl p-6 w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-gray-500 mb-1">Reseller Name *</label>
                <input value={editing.resellerName || ''} onChange={e => setEditing({ ...editing, resellerName: e.target.value })}
                  className="w-full border rounded-lg px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-xs text-gray-500 mb-1">Reseller Code *</label>
                <input value={editing.resellerCode || ''} onChange={e => setEditing({ ...editing, resellerCode: e.target.value })}
                  className="w-full border rounded-lg px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-xs text-gray-500 mb-1">Reseller Org</label>
