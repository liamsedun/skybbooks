  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-ink-900">White Label Config</h1>
          <p className="text-sm text-ink-500 mt-1">Manage branded portal settings per organization</p>
        </div>
        ...
        {isLoading ? (
          ...
          <table className="w-full text-sm">
            <thead><tr className="border-b bg-surface-subtle">
              ...
              <th className="text-left p-3 font-medium text-ink-600">Brand</th>
              <th className="text-left p-3 font-medium text-ink-600">Colors</th>
              <th className="text-left p-3 font-medium text-ink-600">Domain</th>
              <th className="text-center p-3 font-medium text-ink-600">Active</th>
              <th className="text-right p-3 font-medium text-ink-600">Actions</th>
            </tr></thead>
            <tbody>
              {data?.map((item: any) => (
                <tr key={item.id} className="border-b hover:bg-surface-hover">
                  <td className="p-3 text-ink-600">{item.orgName || item.orgId?.slice(0,8)}</td>
                  <td className="p-3">
                    <div className="font-medium text-ink-900">{item.brandName || '-'}</div>
                    <div className="text-xs text-ink-400">{item.supportEmail || '-'}</div>
                  </td>
                  <td className="p-3">
                  ...
                  <td className="p-3 text-xs text-ink-500">{item.customDomain || '-'}</td>
                  <td className="p-3 text-center">
                    <span className={`inline-block w-2 h-2 rounded-full ${item.isActive ? 'bg-green-500' : 'bg-gray-300'}`} />
                  </td>
                  <td className="p-3 text-right">
                    <button onClick={() => openEdit(item)} className="p-1 hover:bg-surface-hover rounded mr-1"><Pen className="w-4 h-4 text-ink-500" /></button>
                    <button onClick={() => setDeleteId(item.id)} className="p-1 hover:bg-red-50 rounded"><Trash2 className="w-4 h-4 text-red-400" /></button>
                  </td>
                </tr>
              ))}
              {(!data || data.length === 0) && <tr><td colSpan={6} className="p-6 text-center text-ink-400">No white label configs</td></tr>}
            </tbody>
          </table>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setEditing(null)}>
          <div className="bg-surface rounded-xl p-6 w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-ink-500 mb-1">Organization *</label>
              ...
              <div>
                <label className="block text-xs text-ink-500 mb-1">Brand Name</label>
                <input value={editing.brandName || ''} onChange={e => setEditing({ ...editing, brandName: e.target.value })}
                  className="w-full border rounded-lg px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-xs text-ink-500 mb-1">Logo URL</label>
                <input value={editing.logoUrl || ''} onChange={e => setEditing({ ...editing, logoUrl: e.target.value })}
                  className="w-full border rounded-lg px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-xs text-ink-500 mb-1">Favicon URL</label>
                <input value={editing.faviconUrl || ''} onChange={e => setEditing({ ...editing, faviconUrl: e.target.value })}
                  className="w-full border rounded-lg px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-xs text-ink-500 mb-1">Primary Color</label>
                <div className="flex items-center gap-2">
              ...
              <div>
                <label className="block text-xs text-ink-500 mb-1">Secondary Color</label>
                <div className="flex items-center gap-2">
              ...
              <div>
                <label className="block text-xs text-ink-500 mb-1">Accent Color</label>
                <div className="flex items-center gap-2">
