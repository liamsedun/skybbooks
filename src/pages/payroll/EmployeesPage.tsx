  ...
  return (
    <div className={span2 ? 'col-span-2' : ''}>
      ...
      <input type={type} value={value} onChange={e => onChange(e.target.value)}
        ...
        className="w-full px-3 py-2.5 text-sm border border-slate-200 rounded-xl bg-slate-50 focus:bg-white focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 outline-none transition-shadow" />
    </div>
  );
}
              ...
              f.type === 'select'
                ? <div key={f.key} className={f.span2 ? 'col-span-2' : ''}>
                    ...
                    <select value={item[f.key]} onChange={e => updateItem(i, f.key, e.target.value)}
                      className="w-full px-3 py-2.5 text-sm border border-slate-200 rounded-xl bg-white focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 outline-none transition-shadow">
                      <option value="">— Select —</option>
...
export function EmployeesPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-6 space-y-6 animate-fade-in">
        ...
        <div className="flex items-center flex-wrap gap-2">
          {selectedIds.length > 0 && (
            <button onClick={() => { if (confirm(`Delete ${selectedIds.length} selected employee(s)?`)) batchDeleteMutation.mutate(selectedIds); }}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-rose-600 bg-rose-50 border border-rose-200 rounded-xl transition-all duration-200 hover:bg-rose-100">
          ...
          <button onClick={() => setImportOpen(true)}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-indigo-600 border border-indigo-200 rounded-xl transition-all duration-200 hover:bg-indigo-50 hover:border-indigo-300">
      ...
      {/* Filters */}
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-5 flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-[200px]">
          <Search className="w-4 h-4 text-slate-400 absolute left-3 top-2.5" />
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search name, staff ID, email..."
            className="w-full pl-9 pr-3 py-2.5 text-xs border border-slate-200 rounded-xl bg-slate-50 focus:bg-white focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 outline-none transition-shadow" />
        </div>
        <select value={filterDept} onChange={e => setFilterDept(e.target.value)}
          className="px-3 py-2.5 text-xs border border-slate-200 rounded-xl bg-slate-50 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 outline-none transition-shadow">
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <button onClick={() => setImportOpen(true)}
            className="p-2 border border-slate-200 bg-white hover:bg-slate-50 rounded-xl text-slate-500 outline-none transition-all duration-200">
              ...
              <table className="w-full text-left text-[13px]">
                <thead className="bg-slate-50">
                  <tr className="bg-slate-50 text-[11px] font-semibold text-slate-400 uppercase tracking-wider">
                    ...
                    <th className="px-3 py-3 text-left">Gross Salary</th>
                ...
                <tbody className="divide-y divide-slate-50">
                  {employees.map((emp: any) => (
                    ...
                    <td className="px-4 py-3">
                      <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold border border-slate-200 bg-slate-100 text-slate-600 capitalize">{emp.paymentFrequency}</span>
                    </td>
                    <td className="px-4 py-3">
                      <button onClick={() => toggleActiveMutation.mutate({ id: emp.id, isActive: !emp.isActive })}
                        className={`inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-xl border transition-all duration-200 ${
                          emp.isActive
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition">
                        <button onClick={() => openEdit(emp)}
                          className="p-1.5 hover:bg-slate-100 text-slate-400 hover:text-indigo-600 rounded-lg transition-all duration-200" title="Edit">
                          <Edit2 className="w-3.5 h-3.5" />
                        </button>
                        <button onClick={() => { if (confirm(`Delete employee ${emp.firstName} ${emp.lastName}?`)) deleteMutation.mutate(emp.id); }}
                          className="p-1.5 hover:bg-red-50 text-slate-400 hover:text-red-600 rounded-lg transition-all duration-200" title="Delete">
                          <Trash2 className="w-3.5 h-3.5" />
                        </button>
                      </div>
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="relative ml-auto w-full max-w-2xl bg-white h-full overflow-y-auto shadow-2xl flex flex-col">
            <div className="sticky top-0 bg-white border-b border-slate-100 px-6 py-4 flex items-center justify-between z-10">
              ...
              <button onClick={resetForm} className="p-2 hover:bg-slate-100 rounded-lg text-slate-400 outline-none transition-all duration-200">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="flex-1 p-6 space-y-6">
              ...
              <div className="sticky bottom-0 bg-white border-t border-slate-100 pt-4 flex gap-3 pb-2">
                <button type="button" onClick={resetForm}
                  className="flex-1 px-4 py-2.5 border border-slate-200 text-slate-600 text-sm font-semibold rounded-xl hover:bg-slate-100 transition-all duration-200">
                  Cancel
                </button>
