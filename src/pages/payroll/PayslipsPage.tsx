                ...
                <tr key={line.id} className="hover:bg-slate-50">
                  ...
                  <td className="py-2.5 pl-2 pr-4">
                    <div className="flex items-center gap-1">
                      ...
                      <button onClick={async () => { try { const blob = await payrollApi.getPayslipPdf(selectedRunId, line.employeeId); downloadBlob(blob, `payslip_${line.employee?.staffId || line.employeeId}.pdf`); } catch (e) { alert('Failed to export PDF.'); console.error(e); } }}
          ...
          <div className="fixed top-0 right-0 h-full w-full max-w-lg bg-white shadow-2xl z-50 flex flex-col">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={async () => { try { const { line } = viewingPayslip; const blob = await payrollApi.getPayslipPdf(selectedRunId, line.employeeId); downloadBlob(blob, `payslip_${line.employee?.staffId || line.employeeId}.pdf`); } catch (e) { alert('Failed to export PDF.'); console.error(e); } }} className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-blue-700 bg-blue-50 hover:bg-blue-100 rounded-lg">
