...
- **Email SMTP settings**: Full SMTP configuration page at `/settings/email-settings` with nodemailer integration; test email button; fixed IPv6 `ENETUNREACH` via `dns.resolve4()` to resolve hostname to IPv4 before connecting; added explicit connection/greeting/socket timeouts (30s/15s/60s); autocomplete attributes on form fields
