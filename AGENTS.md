...
- **Critical — coupon/promotion cross-org leak**: Added `orgId` parameter + `or(eq(orgId, ?), isNull(orgId))` scoping to `getCoupon()`, `validateCoupon()`, `getPromotion()`, `validatePromotion()` in `subscription.service.ts`. Updated all 6 call sites (3 internal + 3 routes)
- **Critical — lease CRUD cross-org access**: 9 functions in `lease.service.ts` (`getLease`, `updateLease`, `processLeasePayment`, `postLeaseDepreciation`, `postCommencementEntry`, `modifyLease`, `terminateLease`, `batchProcessPayments`, `batchPostDepreciation`) now pass `orgId`; 9 routes in `leases.ts` propagate `req.user!.orgId!`
- **High — banking UPDATE/DELETE unguarded**: 7 queries on `bankAccounts`, `bankRules`, `bankTransfers` in `routes/banking.ts` now include orgId WHERE clause
- **High — journal status transition unguarded**: 6 UPDATE queries (submit-review, approve, post, lock, cancel) in `routes/journals.ts` now include orgId WHERE clause
- **Medium — revenue schedule org verification**: `getSchedules()` and `addManualSchedule()` in `revenue.service.ts` verify obligation belongs to requesting org via contract join
- **Medium — getAccountBalance orgId**: Added `orgId` as first parameter to `getAccountBalance()` in `ledger.service.ts`; updated single caller in `reconciliation.service.ts`
- **Medium — invoice item/lot orgId**: Item lookup (`items.id`) and inventory lot lookup (`inventoryLots.itemId`) in `createInvoiceJournalEntry()` now filter by `eq(orgId)`
- **Low — getEclDetail orgId**: Added optional `orgId` filter to `getEclDetail()` in `ecl.service.ts`; updated caller in `routes/ecl.ts`
- **All changes compile**: `npx tsc --noEmit` passes with 0 errors
- **Committed at `d5b6f0b`**: "Multi-tenancy hardening: orgId scoping fixes across 15 files"
